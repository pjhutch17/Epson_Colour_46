...
DITHERDATA_T *CreateDitherData(LONG samplesPerLine,LONG numLines,LONG pad)
{
	...
	dd = IExec->AllocVecTags(sizeof(*dd) +
...
VOID RotateDitherLines(DITHERDATA_T * dd)
{
	...
	memset(dd->dd_Lines[dd->dd_NumLines-1],0,(size_t)(sizeof(SAMPLE_T) * dd->dd_SamplesPerLine));
	...
	int component)		 /* Which colour component */
{
	...
	/* Are we thresholding? */
	if(PInfo->pi_threshold > 0 && PD->pd_Preferences.PrintShade == SHADE_BW)
	{
		TransferThreshold(ptr,PInfo->pi_width,ColorInt,component, PInfo->pi_threshold);
	}
	else
	{
